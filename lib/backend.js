...
export async function getAcoes () {
  ...
  const docQuery = await api.query(Prismic.Predicates.any('document.type', ['article', 'story', 'pictures_and_video', 'post']), { fetchLinks: ['author.name', 'category.name'], orderings : '[my.article.date, my.story.date, my.pictures_and_video.date]' })
  const catQuery = await api.query(Prismic.Predicates.any('document.type', ['category']), { orderings : '[my.category.name desc]' })
